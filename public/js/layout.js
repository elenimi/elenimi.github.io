  ...
  var Page = (function () {
    var $container = $('#hs-container'),
      ...
      _goto = function (section) {
        ...
        const url_sec = History.getState().url.split('/').pop();
        const sec = section_map[url_sec ? url_sec : 'about'];
        var section = section || sec;
        var isHome = section === undefined;
        ...
        if ($article.length) {
          ...
          var is_v = $(document).height() - $(window).height() > 0;
          ...
          var param = is_v
            ...
            : { scrollLeft: left };
          var $elScroller = is_v ? $('html, body') : $scroller;
      ...
      _saveState = function (section) {
        ...
        const sec = section_map[url_sec ? url_sec : 'about'];
        if (sec !== section) {
          const new_url_sec = reverse_section_map[section - 1];
          History.pushState(null, null, new_url_sec);
        }
      },
      _layout = function () {
      ...
      _initMenuEvents = function () {
        ...
        $links.on('click', function (event) {
          ...
          var section = href.search(/section/) !== -1 ? href.substring(8) : 0;
          _saveState(section);
      ...
      _initArticleEvents = function () {
        ...
        $container.on('click', 'article.hs-content', function (event) {
          var id = $(this).attr('id');
          var section = id.search(/section/) !== -1 ? id.substring(7) : 0;
          _saveState(section);
        });
      },
